...
export const Navbar: React.FC<NavbarProps> = ({ onOpenEnquiry, onOpenBrochure, onOpenShare }) => {
  ...
  const scrollTo = (id: string) => {
    ...
    if (element) {
      ...
      const headerOffset = 70 + stagingOffset;
  ...
  return (
    <>
      <header
        className={`roopkund-fixed-header fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="roopkund-mobile-drawer fixed inset-0 z-40 bg-[#0B131F]/98 backdrop-blur-xl flex flex-col justify-between p-6 sm:p-8 pt-20 text-white lg:hidden animate-in fade-in duration-200 overflow-y-auto border-b border-white/10">
          ...
          <div className="space-y-2.5 pt-4 border-t border-white/10">
            ...
            <div className="text-center text-[10px] text-white/50 tracking-wider font-oswald pt-1">
              PRAVAAH TRAVELS • DETAILS UNDER REVIEW
            </div>
          </div>
        </div>
      )}

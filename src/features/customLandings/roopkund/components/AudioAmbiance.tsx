...
export const AudioAmbiance: React.FC<AudioAmbianceProps> = ({ theme = 'light' }) => {
  ...
  const startAmbiance = () => {
    try {
      ...
      // Pink Noise Generator (Simulating Himalayan Alpine Wind)
      ...
      // Gentle lowpass filter simulating mountain breeze
      ...
      // Soft harmonic Himalayan bell drone
      const bellOsc = ctx.createOscillator();
      bellOsc.type = 'sine';
      bellOsc.frequency.setValueAtTime(216, ctx.currentTime); // 216Hz sacred mountain resonance
  ...
  return (
    <button
      ...
      title={isPlaying ? 'Mute Himalayan Mountain Atmosphere' : 'Play Himalayan Mountain Atmosphere (Wind & Bell Resonance)'}
      aria-label="Toggle Alpine Ambiance Sound"
    >
      {isPlaying ? (
        <>
        ...
        <>
          ...
          <span className="hidden sm:inline">Himalayan Audio</span>
        </>
      )}
    </button>
  );

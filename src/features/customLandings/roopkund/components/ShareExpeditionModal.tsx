...
export const ShareExpeditionModal: React.FC<ShareExpeditionModalProps> = ({
  ...
  const expeditionTitle = 'Roopkund Trek (15,750 FT) — 8-Day Mystery Trail Expedition | Pravaah';
  const expeditionSummary = 'Explore the ancient high-altitude glacial lake at 15,750 FT beneath Mt. Trishul with Asia’s largest meadows, 1:6 leader-to-trekker ratio, and sustainable mountain stewardship.';
  const shareUrl = typeof window !== 'undefined' ? window.location.href : 'https://pravaahtravels.com/expeditions/roopkund';
  ...
  const handleWhatsAppShare = () => {
    const text = encodeURIComponent(`🏔️ *${expeditionTitle}*\n\n${expeditionSummary}\n\n👉 Discover the full 8-Day Day-by-Day Itinerary & Acclimatization profile:\n${shareUrl}`);
    window.open(`https://api.whatsapp.com/send?text=${text}`, '_blank');
  };

  const handleTwitterShare = () => {
    const text = encodeURIComponent(`Walking the ancient Himalayan mystery trail to Roopkund Lake at 15,750 FT with @PravaahTravels. 🏔️✨ Check out the full 8-day expedition dossier:`);
  ...
  const handleEmailShare = () => {
    const subject = encodeURIComponent(`Roopkund Himalayan Expedition (15,750 FT) — 8-Day Trail Dossier`);
    const body = encodeURIComponent(`Hi,\n\nI thought you might be interested in this 8-day Himalayan expedition to Roopkund Lake (15,750 FT) with Pravaah:\n\n${expeditionSummary}\n\nCheck out the full itinerary, elevation maps, and gear checklist here:\n${shareUrl}\n\nWarm regards!`);
      ...
      >
        ...
        <div className="p-6 sm:p-8 bg-white border-b border-[#E2DDD3]">
          ...
          <p className="mt-1.5 text-xs sm:text-sm font-nunito text-[#4A5568] leading-relaxed">
            Share the Roopkund Mystery Trail with your trekking partners, friends, or family before finalizing your batch.
          </p>
        </div>

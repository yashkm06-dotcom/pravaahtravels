...
export default function TravelMedia({
  ...
  referrerPolicy = 'no-referrer', width, height, disableFallback = false,
}: TravelMediaProps) {
  const trimmedSrc = typeof src === 'string' ? src.trim() : '';
  const [hasError, setHasError] = useState(false);

  useEffect(() => setHasError(false), [disableFallback, trimmedSrc]);
  ...
  if (isWebmMediaUrl(trimmedSrc)) {
    return <video src={mediaUrl} className={className} autoPlay muted loop playsInline
      preload={loading === 'eager' ? 'auto' : 'metadata'} aria-label={alt} title={alt}
      onError={disableFallback ? () => setHasError(true) : undefined} />;
  }

  return <img src={mediaUrl} alt={alt} className={className} referrerPolicy={referrerPolicy}
    loading={loading} decoding={decoding} width={width} height={height}

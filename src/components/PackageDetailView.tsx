...
interface PackageDetailViewProps { pkg: TravelPackage; onBack: () => void; onEnquirySuccess: () => void; isAdminLoggedIn?: boolean; onDeletePackage?: (id: string) => void; onPackageSaved?: () => void; wishlistPackageIds?: string[]; onToggleWishlist?: (pkg: TravelPackage) => void; onNavigate?: (view: string, packageId?: string | null) => void; packages?: TravelPackage[]; websiteCMS?: WebsiteCMSSettings; googleReviews?: { rating?: number; totalReviews?: number } | null; }

const imageList = (pkg: TravelPackage) => Array.from(new Set([getPackageVisual(pkg), pkg.packageBannerUrl, pkg.heroImage, pkg.imageUrl, ...(pkg.galleryImages || []), ...(pkg.gallery || [])].filter((value): value is string => Boolean(value && /^(https?:\/\/|\/|data:image\/)/i.test(value) && !/vitour-travel-placeholder|placeholder\.(jpg|jpeg|png|webp)/i.test(value)))));
const initialForm = { name: '', phone: '', email: '', whatsapp: '', travelDate: '', travelers: 2, budget: 'Rs. 20,000 - Rs. 50,000', message: '', preferredAccommodation: '', pickupLocation: '' };
...
function AccordionContent({ open, children }: { open: boolean; children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const update = () => setHeight(open ? content.scrollHeight : 0);
    update();
  ...
  return <div className={`pravaah-accordion-content ${open ? 'is-open' : ''}`} style={{ height: `${height}px` }} aria-hidden={!open}><div ref={contentRef}>{children}</div></div>;
}

const handleGalleryImageError = (event: SyntheticEvent<HTMLImageElement>, fallbackSource?: string) => {
  const image = event.currentTarget;
  image.onerror = null;
  const fallback = getTravelImage(fallbackSource);
  if (image.src !== fallback) image.src = fallback;
};

export default function PackageDetailView({ pkg, onBack, onEnquirySuccess, isAdminLoggedIn = false, onDeletePackage, onPackageSaved, wishlistPackageIds = [], onToggleWishlist, onNavigate, packages = [], websiteCMS }: PackageDetailViewProps) {
  ...
  useEffect(() => {
    ...
    const loadReviews = async () => {
      ...
      try {
        ...
        const current = String(pkg.title).toLowerCase();
        const matching = snapshot.docs.map((item) => ({ id: item.id, ...item.data() }) as Review).filter((review) => (review.status || 'Approved') === 'Approved' && (String(review.destination || '').toLowerCase().includes(current) || String(pkg.destination || '').toLowerCase().includes(String(review.destination || '').toLowerCase()))).slice(0, 4);
        if (!cancelled) setReviews(matching);
      } catch { if (!cancelled) setReviews([]); } finally { if (!cancelled) setReviewsLoading(false); }
    };
    void loadReviews();
    return () => { cancelled = true; };
  }, [pkg.destination, pkg.title]);

  const related = useMemo(() => packages.filter((item) => item.active !== false && item.id !== pkg.id && (item.category === pkg.category || item.location === pkg.location)).slice(0, 3), [packages, pkg.category, pkg.id, pkg.location]);
  const reviewAverage = reviews.length ? reviews.reduce((sum, review) => sum + Number(review.rating || 0), 0) / reviews.length : 0;
  const reviewCount = reviews.length;
  const routeDetails = [pkg.transportType, ...(pkg.mealPlans || [])].filter(Boolean);
  const saved = wishlistPackageIds.includes(String(pkg.id));
  const whatsappUrl = business.whatsappUrl(`Hello ${business.companyName}, I would like details for ${pkg.title}.`);
  const updateField = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { const { name, value } = event.target; setFormData((prev) => ({ ...prev, [name]: name === 'travelers' ? Math.max(1, Number(value) || 1) : value })); };
  const submitEnquiry = async (event: React.FormEvent) => {
    event.preventDefault(); setFormError('');
    if (!formData.name.trim() || !formData.phone.trim() || !formData.email.trim() || !formData.travelDate) { setFormError('Please complete your name, phone, email, and travel date.'); return; }
    setSubmitting(true);
    const message = formData.message || `Hi, I am interested in the "${pkg.title}" package. Please share details.`;
    try {
      const route = getPackageNavigationTarget(pkg);
      const payload: Omit<Enquiry, 'id'> = { name: formData.name.trim(), phone: formData.phone.trim(), email: formData.email.trim(), whatsapp: formData.whatsapp.trim() || formData.phone.trim(), destination: pkg.destination, travelDate: formData.travelDate, travelers: formData.travelers, budget: formData.budget, message, packageId: pkg.id, packageName: pkg.title, packageSlug: route.path, preferredAccommodation: formData.preferredAccommodation, pickupLocation: formData.pickupLocation, sourceUrl: window.location.href, adults: formData.travelers, status: 'New', createdAt: new Date().toISOString() };
      await addDoc(collection(db, 'enquiries'), payload);
      const emailPayload = { name: formData.name, email: formData.email, phone: formData.phone, destination: pkg.destination, travelDate: formData.travelDate, travelers: formData.travelers, budget: formData.budget, message };
      triggerSystemEmail('enquiry-received', formData.email, emailPayload);
      triggerSystemEmail('new-enquiry', 'yash.km06@gmail.com', emailPayload);
      setSuccess(true); setFormData(initialForm); onEnquirySuccess();

...
function PackageResult({ pkg, index, saved, onNavigate, onToggleWishlist }: { pkg: TravelPackage; index: number; saved: boolean; onNavigate: PackagesViewProps['onNavigate']; onToggleWishlist?: PackagesViewProps['onToggleWishlist'] }) {
  ...
  return <article className={`pravaah-catalog-item pravaah-catalog-item--${(index % 3) + 1}`}>
    <button type="button" className="pravaah-catalog-item__image" onClick={() => openPackage(onNavigate, pkg)} aria-label={`Open ${pkg.title}`}><PackageImage src={image} alt={pkg.title} className="h-full w-full object-cover" /><span className="pravaah-catalog-item__number">{String(index + 1).padStart(2, '0')}</span>{pkg.offerPrice && pkg.offerPrice < pkg.price && <span className="pravaah-catalog-item__offer">Offer</span>}</button>
    <div className="pravaah-catalog-item__body"><div className="pravaah-catalog-item__meta"><span>{pkg.category}</span><span>{pkg.location || pkg.destination}</span></div><h2><button type="button" onClick={() => openPackage(onNavigate, pkg)}>{pkg.title}</button></h2><p>{pkg.shortDescription || 'A considered journey through remarkable country.'}</p><div className="pravaah-catalog-item__facts"><span><Clock3 className="h-4 w-4" aria-hidden="true" />{pkg.duration}</span><span><Users className="h-4 w-4" aria-hidden="true" />{pkg.maxGuests ? `Up to ${pkg.maxGuests}` : 'Small group'}</span><span className="pravaah-catalog-item__rating"><Star className="h-4 w-4" aria-hidden="true" />{rating > 0 ? `${rating.toFixed(1)} guest rating` : 'Guest rating pending'}</span></div><div className="pravaah-catalog-item__footer"><span>From <strong>{formatPackagePrice(pkg.offerPrice || pkg.price)}</strong></span><div>{onToggleWishlist && <button type="button" className={`pravaah-save-button ${saved ? 'is-saved' : ''}`} onClick={() => onToggleWishlist(pkg)} aria-label={saved ? `Remove ${pkg.title} from saved journeys` : `Save ${pkg.title}`} title="Save journey"><Heart className="h-4 w-4" aria-hidden="true" /></button>}<button type="button" className="pravaah-outline-button pravaah-outline-button--small" onClick={() => openPackage(onNavigate, pkg)}>View route <ArrowUpRight className="h-4 w-4" aria-hidden="true" /></button></div></div></div>
  </article>;
}

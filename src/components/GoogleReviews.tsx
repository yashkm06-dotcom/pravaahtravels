    ...
    <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
      ...
      {reviews.length > 0 ? <>
        <div className="mx-auto mt-6 w-full max-w-[980px]" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPaused(false); }}>
          ...
          {reviews.length > 1 && <div className="mt-4 flex justify-center gap-2" aria-label="Google review carousel pages">{reviews.map((review, index) => <button key={`${review.authorName || 'review'}-${index}`} type="button" aria-label={`Show Google review ${index + 1}`} aria-current={index === currentIndex ? 'true' : undefined} onClick={() => goTo(index)} className={`h-2 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#4da528] focus:ring-offset-2 ${index === currentIndex ? 'w-7 bg-[#4da528]' : 'w-2 bg-[#d8d3c9] hover:bg-[#9ca690]'}`} />)}</div>}
        </div>
